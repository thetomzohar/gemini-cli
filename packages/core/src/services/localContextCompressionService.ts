    ...
    try {
      ...
      const resp = await fetchFn(modelUrl, {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: 'user',
            content: `Summarize this file in 2-3 sentences. Be technical and specific about what it exports, its key functions, and dependencies. File: ${filepath}\n\n${content.slice(0, 4000)}`,
          }],
        }),
      });
      const data = await resp.json();
      ...
      return `[Summary generation failed for ${filepath}]`;
    }
  }
}
