  ...
  describe('compressHistory', () => {
    ...
    it('protects files that were read within the RECENT_TURNS_PROTECTED window', async () => {
      const history: Content[] = [
         // Turn 0 & 1 (Old)
         { role: 'model', parts: [{ functionCall: { name: 'read_file', args: { filepath: 'src/app.ts' } } }] },
         { role: 'user', parts: [{ functionResponse: { name: 'read_file', response: { output: '--- src/app.ts ---\nLine 1\nLine 2\nLine 3' } } }] },
    ...
    it('compresses files read outside the protected window', async () => {
      const history: Content[] = [
         ...
         { role: 'model', parts: [{ functionCall: { name: 'read_file', args: { filepath: 'src/old.ts' } } }] },
         // Turn 1: The tool output response
         { role: 'user', parts: [{ functionResponse: { name: 'read_file', response: { output: '--- src/old.ts ---\nLine 1\nLine 2\nLine 3\nLine 4' } } }] },
       ...
       mockFetch.mockResolvedValueOnce({
         ok: true,
         json: async () => ({
            choices: [{ message: { content: JSON.stringify({ level: 'PARTIAL', start_line: 2, end_line: 3 }) } }]
         })
       });
    ...
    it('returns SUMMARY and hits cache on subsequent requests', async () => {
       ...
       const history1: Content[] = [
         { role: 'model', parts: [{ functionCall: { name: 'read_file', args: { filepath: 'src/index.ts' } } }] },
         { role: 'user', parts: [{ functionResponse: { name: 'read_file', response: { output: `--- src/index.ts ---\nVery long content here...` } } }] },
       ...
       mockFetch.mockResolvedValueOnce({
         ok: true,
         json: async () => ({
            choices: [{ message: { content: JSON.stringify({ level: 'SUMMARY' }) } }]
         })
       });
